...
class ConfirmBookingAction
{
    ...
    /**
     * Confirm a booking by assigning room units to each booking room.
    ...
    public function execute(Booking $booking): bool
    {
        return DB::transaction(function () use ($booking) {
            ...
            foreach ($booking->bookingRooms as $bookingRoom) {
                // Skip if already assigned a unit
                if ($bookingRoom->room_unit_id) {
                    Log::info("Booking room {$bookingRoom->id} already has unit {$bookingRoom->room_unit_id} assigned");
                    continue;
                }

                // Try to assign a room unit
