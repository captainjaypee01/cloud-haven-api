                                                ...
                                                @if($selection['include_lunch'] && $selection['lunch_cost'] > 0)
                                                    <div style="margin-bottom: 4px;">
                                                        ...
                                                        @if($selection['adults'] > 0 && $selection['children'] > 0)
                                                            {{ $selection['adults'] }} adults × {{ $fmtMoney($selection['adult_lunch_price'] ?? 0) }} + {{ $selection['children'] }} children × {{ $fmtMoney($selection['child_lunch_price'] ?? 0) }} = {{ $fmtMoney($selection['lunch_cost']) }}
                                                        @elseif($selection['adults'] > 0)
                                                            {{ $selection['adults'] }} adults × {{ $fmtMoney($selection['adult_lunch_price'] ?? 0) }} = {{ $fmtMoney($selection['lunch_cost']) }}
                                                        @else
                                                            {{ $selection['children'] }} children × {{ $fmtMoney($selection['child_lunch_price'] ?? 0) }} = {{ $fmtMoney($selection['lunch_cost']) }}
                                                        @endif
                                                    </div>
                                                @else
                                            ...
                                            <div style="font-size: 13px; color: #6b7280; margin-left: 12px;">
                                                @if($selection['include_pm_snack'] && $selection['pm_snack_cost'] > 0)
                                                    <div style="margin-bottom: 4px;">
                                                        ...
                                                        @if($selection['adults'] > 0 && $selection['children'] > 0)
                                                            {{ $selection['adults'] }} adults × {{ $fmtMoney($selection['adult_pm_snack_price'] ?? 0) }} + {{ $selection['children'] }} children × {{ $fmtMoney($selection['child_pm_snack_price'] ?? 0) }} = {{ $fmtMoney($selection['pm_snack_cost']) }}
                                                        @elseif($selection['adults'] > 0)
                                                            {{ $selection['adults'] }} adults × {{ $fmtMoney($selection['adult_pm_snack_price'] ?? 0) }} = {{ $fmtMoney($selection['pm_snack_cost']) }}
                                                        @else
                                                            {{ $selection['children'] }} children × {{ $fmtMoney($selection['child_pm_snack_price'] ?? 0) }} = {{ $fmtMoney($selection['pm_snack_cost']) }}
                                                        @endif
                                                    </div>
                                                @else
                                                    <div style="margin-bottom: 4px; color: #9ca3af;">
