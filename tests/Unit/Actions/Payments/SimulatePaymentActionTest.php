...
describe('SimulatePaymentAction', function () {
    it('returns success for outcome=success', function () {
        ...
        $dto = new PaymentRequestDTO('NTDL-' . now()->format('ymd') . '-' . strtoupper(Str::random(6)), 1000, 'simulation', 'success');
    ...
    it('returns failure for outcome=fail', function () {
        $action = new SimulatePaymentAction();
        $dto = new PaymentRequestDTO('NTDL-' . now()->format('ymd') . '-' . strtoupper(Str::random(6)), 1000, 'simulation', 'fail');
        $result = $action->execute($dto);
